...
print("Starts training XGBoost phishing")
acc_list = []
f1_list = []
...
for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
    start = datetime.now()
    print("Cross Validation Fold {}".format(i))
    model_name = "splitnn_phishing_train_party_{}_fold_{}".format(num_parties, i)
...
for i, result in enumerate(f1_summary):
    mean = np.mean(result)
    std = np.std(result)
    print("Party {}: F1 mean={}, std={}".format(i, mean, std))
...
for party_id in range(num_parties):
    ...
    for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
        print("Cross Validation Fold {}".format(i))
        name = "single_phishing_train_party_{}_single_{}_fold_{}".format(num_parties, party_id, i)
        writer = SummaryWriter("runs/{}".format(name))
...
for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
    print("Cross Validation Fold {}".format(i))
    model_name = "post_fedonce_phishing_train_party_{}_fold_{}".format(num_parties, i)
    name = "{}/".format(model_name)
    active_party = 0
    cache_local_name = "fedonce_phishing_party_{}_fold_{}".format(num_parties, i)
...
for active_party in range(num_parties):
    ...
    for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
        ...
        acc, f1, _, _ = aggregate_model.train(xs_train, y_train, xs_val, y_val, use_cache=False)
        y_score_test = aggregate_model.predict_agg(xs_test)
        y_pred_test = np.where(y_score_test > 0.5, 1, 0)
        test_acc = accuracy_score(y_test, y_pred_test)
        test_f1 = f1_score(y_test, y_pred_test)
