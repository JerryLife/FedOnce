...
xs_train_val, y_train_val, xs_test, y_test, x_train, x_test = \
    load_data_train_test("kmnist", "kmnist", num_parties=num_parties, file_type='torch')
acc_summary = []
f1_summary = []
acc_list = []
f1_list = []
...
for i, (train_idx, val_idx) in enumerate(KFold(n_splits=5, shuffle=True).split(y_train_val)):
    ...
    model_name = "splitnn_kmnist_party_{}_fold_{}".format(num_parties, i)
...
for active_party in range(num_parties):
    ...
    for i, (train_idx, val_idx) in enumerate(kfold):
        ...
        xs_val = [data[val_idx] for data in xs_train_val]
        y_val = y_train_val[val_idx]
        model_name = "fedonce_kmnist_party_{}_fold_{}".format(num_parties, i)
...
for party_id in range(num_parties):
    ...
    for i, (train_idx, val_idx) in enumerate(KFold(n_splits=5, shuffle=True).split(y_train_val)):
        ...
        xs_val = [data[val_idx] for data in xs_train_val]
        y_val = y_train_val[val_idx]
        name = "single_kmnist_party_{}_single_{}_fold_{}".format(num_parties, party_id, i)
        writer = SummaryWriter("runs/{}".format(name))
...
for party_id in range(num_parties):
    ...
    for i, (train_idx, val_idx) in enumerate(kfold):
        ...
        xs_val = [data[val_idx] for data in xs_train_val]
        y_val = y_train_val[val_idx]
        name = "combine_kmnist_fold_{}".format(i)
        writer = SummaryWriter("runs/{}".format(name))
        single_model = SingleParty(
            party_id=party_id,
            num_epochs=200,
...
for i, (train_idx, val_idx) in enumerate(KFold(n_splits=5, shuffle=True).split(y_train_val)):
    ...
    model_name = "post_fedonce_kmnist_party_{}_fold_{}".format(num_parties, i)
    name = "{}/".format(model_name)
    active_party = 0
    cache_local_name = "fedonce_kmnist_party_{}_fold_{}".format(num_parties, i)
...
for active_party in range(num_parties):
    ...
    for i, (train_idx, val_idx) in enumerate(kfold):
        ...
        xs_val = [data[val_idx] for data in xs_train_val]
        y_val = y_train_val[val_idx]
        model_name = "fedonce_kmnist_party_{}_pca".format(num_parties, i)
        name = "{}_active_{}".format(model_name, active_party)
        writer = SummaryWriter("runs/{}".format(name))
