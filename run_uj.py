...
print("Starts training XGBoost UJ")
rmse_list = []
...
for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
    start = datetime.now()
    print("Cross Validation Fold {}".format(i))
    model_name = "splitnn_uj_party_{}_fold_{}".format(num_parties, i)
...
for party_id in range(num_parties):
    ...
    for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
        print("Cross Validation Fold {}".format(i))
        name = "single_uj_party_{}_single_{}_fold_{}".format(num_parties, party_id, i)
        writer = SummaryWriter("runs/{}".format(name))
        single_model = SingleParty(
            party_id=party_id,
            num_epochs=500,
            lr=3e-4,
            hidden_layers=[50, 50, 30],
            batch_size=100,
            weight_decay=1e-4,
            writer=writer,
            device='cuda:{}'.format(args.gpu),
...
for party_id in range(num_parties):
    ...
    for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
        print("Cross Validation Fold {}".format(i))
        name = "combine_uj_fold_{}".format(i)
        writer = SummaryWriter("runs/{}".format(name))
        single_model = SingleParty(
            party_id=party_id,
            num_epochs=600,
...
for i, (xs_train, y_train, xs_val, y_val) in enumerate(cross_valid_data):
    print("Cross Validation Fold {}".format(i))
    model_name = "post_fedonce_uj_party_{}_fold_{}".format(num_parties, i)
    name = "{}/".format(model_name)
    active_party = 0
    cache_local_name = "fedonce_uj_party_{}_fold_{}".format(num_parties, i)
